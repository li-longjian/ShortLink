...
public interface LinkAccessLogsMapper extends BaseMapper<LinkAccessLogDO> {
    ...
    @Select("SELECT " +
            ...
            "    count DESC " +
            "LIMIT 5;")
    List<HashMap<String, Object>> listTopIpByGroup(@Param("param") LinkStatsGroupReqDTO requestParam);
    /**
     * 根据短链接获取指定日期内新旧访客数据
     */
    @Select("SELECT " +
            ...
            "        user " +
            ") AS user_counts;")
    HashMap<String, Object> findUvTypeCntByGroup(@Param("param")LinkStatsGroupReqDTO requestParam);
    /**
     * 根据短链接获取指定日期内新旧访客数据
     */
    @Select("SELECT " +
    ...
    @Select("select count(user) as pv, count(distinct  user) as uv, count(distinct  ip ) as uip\n " +
            "from t_link_access_logs\n " +
            "where gid = #{gid}  and create_time between #{startDate} and #{endDate}\n " +
            "group by gid")
    ShortLinkStatsDO findPvUvUipByGroup(LinkStatsGroupReqDTO requestParam);
